...
# VPS Manager by AndrewON Version 1.2
...
RESET_BACKUP_DIR="$HOME/reset_backup"
...
# Function to backup files before resetting
backup_reset_files() {
    echo -e "${GREEN}Backing up files before resetting...${NC}"
    if [ ! -d "$RESET_BACKUP_DIR" ]; then
        mkdir -p "$RESET_BACKUP_DIR"
    fi

    # Backup files that will be deleted during reset
    cp -r /var/www "$RESET_BACKUP_DIR/var_www" 2>/dev/null
    cp -r /etc/pterodactyl "$RESET_BACKUP_DIR/etc_pterodactyl" 2>/dev/null
    cp -r /var/lib/pterodactyl "$RESET_BACKUP_DIR/var_lib_pterodactyl" 2>/dev/null

    echo -e "${GREEN}Backup of reset files completed successfully!${NC}"
}

# Function to restore files after resetting
restore_reset_files() {
    echo -e "${YELLOW}Restoring files from reset backup...${NC}"
    if [ ! -d "$RESET_BACKUP_DIR" ]; then
        echo -e "${RED}No reset backup found. Please backup before resetting.${NC}"
        return
    fi

    # Restore files that were backed up
    cp -r "$RESET_BACKUP_DIR/var_www"/* /var/www/ 2>/dev/null
    cp -r "$RESET_BACKUP_DIR/etc_pterodactyl"/* /etc/pterodactyl/ 2>/dev/null
    cp -r "$RESET_BACKUP_DIR/var_lib_pterodactyl"/* /var/lib/pterodactyl/ 2>/dev/null

    echo -e "${GREEN}Restore from reset backup completed successfully!${NC}"
}
...
reset_vps() {
    ...
    rm /etc/systemd/system/wings.service 2>/dev/null
    apt-get remove --purge -y wings pterodactyl-panel 2>/dev/null

    # Remove Wings volumes folder
    echo -e "${YELLOW}Removing Wings volumes folder...${NC}"
    rm -rf /var/lib/pterodactyl/volumes 2>/dev/null
...
show_credits() {
    echo -e "${CYAN}==========================${NC}"
    echo -e "${CYAN}Created by AndrewON - Version 1.2${NC}"
    echo -e "${CYAN}==========================${NC}"
}
